...
namespace wp_engine
{
    ...
    class State : StateInterface
    {
        ...
        protected List<ActionObject> methodButtons; /**< Contains buttons that trigger a method */
        ...
        public State(int screenWidth, int screenHeight)
        {
            ...
            methodButtons = new List<ActionObject>();
        ...
        virtual public void loadContent(ContentManager contentManager) {
            ...
            foreach (ActionObject eventobj in methodButtons)
            {
                eventobj.loadAssets(contentManager);
            }
        }
        ...
        public virtual void handleEvent(Event recentEvent) {
            if (recentEvent.getType() == GestureType.Tap)
            {
                foreach (ActionObject actionbutton in methodButtons)
                {
                    if(actionbutton.isPressed(recentEvent.getVector())) {
                        Console.WriteLine("Action button has been tapped!");
                    }
                }
            }            
        }
